 ...
 *
 * Lifecycle:
 *   1. Call local_model_init_params() once to load the model into memory.
 *   2. Call local_model_prompt() for each user message — returns the response.
 *   3. Call local_model_cleanup() when done to free model memory.
 *
 * All functions use C linkage so they can be called from Dart FFI without
 * name mangling. The implementation lives in local_model.cpp.
 *
...
extern "C"
{
    ...
    /**
     * Configuration passed to local_model_init().
     *
    ...
    struct LocalModelConfig
    {
        ...
        int model_type;             // chat template index — see LocalModelType in local_config.dart
    };

    /**
     * Initialises the model from a LocalModelConfig struct.
     *
    ...
    /**
     * Flat-parameter wrapper around local_model_init — easier to call from Dart FFI.
     *
     * Avoids struct layout alignment issues between Dart and C++.
     * Parameters map 1-to-1 to LocalModelConfig fields.
     *
     ...
     *
     * The returned pointer is valid until the next call to local_model_prompt()
     * or local_model_cleanup(). Do NOT free it from Dart.
     *
     ...
     *
     * Call once when the engine is disposed. After this, local_model_init_params()
     * must be called again before using local_model_prompt().
     */
