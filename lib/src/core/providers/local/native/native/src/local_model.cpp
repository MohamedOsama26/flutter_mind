...
static int detect_model_type()
{
    ...
    if (a == "llama")     return 2;
    if (a == "gemma")     return 3;
    if (a == "gemma2")    return 3; // same template as gemma
    if (a == "gemma3")    return 3;
    if (a == "phi2")      return 4;
    if (a == "phi3")      return 4; // same template as phi
    if (a == "mistral")   return 5;
    if (a == "deepseek2") return 6;
    return 0;
}
...
const char *local_model_prompt(const char *prompt)
{
    ...
    if (g_config.top_k > 0)
        llama_sampler_chain_add(sampler, llama_sampler_init_top_k(g_config.top_k));

    if (g_config.top_p > 0)
        llama_sampler_chain_add(sampler, llama_sampler_init_top_p(g_config.top_p, 1));
    ...
    llama_sampler_chain_add(sampler, llama_sampler_init_penalties(64, penalty, 0.0f, 0.0f));
    ...
    for (int i = 0; i < max; i++)
    {
        ...
        // convert token id to text and append
        char buf[128];
        ...
        // user-defined stop sequences — break immediately, don't generate further
        if (ends_with_stop(g_response)) {
            trim_stop(g_response);
            break;
        }

        // feed the generated token back as input for the next step
    ...
    // safety net — catches stop sequences that span a token boundary
    trim_stop(g_response);
...
// individual scalars instead of a struct. Maps 1-to-1 to LocalModelConfig.
